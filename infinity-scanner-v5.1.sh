...
# ─── Safe .env loading (Telegram + Admin credentials only) ───
load_env_safe() {
  if [ -f .env.local ]; then
...
# ─── Obtain JWT token if admin credentials exist ───
AUTH_TOKEN=""
if [ -n "$ADMIN_EMAIL" ] && [ -n "$ADMIN_PASSWORD" ]; then
  ...
  if [ -n "$AUTH_TOKEN" ]; then
    ...
    echo -e "${YELLOW}⚠️  Could not obtain token (check ADMIN_EMAIL/PASSWORD). Scanning without auth.${NC}"
  fi
else
  echo -e "${YELLOW}ℹ️  No admin credentials set – scanning only public endpoints.${NC}"
fi
...
# ─── Test a single URL (now supports auth token) ───
test_one() {
  local method="$1" url="$2" data="$3" desc="$4"
  local tmp=$(mktemp) code dur body auth_header="" use_auth=false
  # For API routes, also test with auth token if available
...
# ─── Main ───
main() {
  echo -e "${PURPLE}⚡ INFINITY GOD MODE SCANNER v5.1 (Auth-Aware)${NC}"
  wait_server || exit 1
  ...
  # Summary stats – exclude AUTH-REQUIRED from real errors
  ...
  # Telegram message (real problems only)
  ...
  if [ "$real_issues" -gt 0 ]; then
    ...
    telegram_msg+="✅ No critical/error/missing issues found!%0A"
  fi
