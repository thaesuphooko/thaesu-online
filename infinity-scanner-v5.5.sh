#!/bin/bash
# ╔══════════════════════════════════════════════════════════╗
# ║  INFINITY SCANNER v5.5 – Premium Error‑Free Edition    ║
# ╚══════════════════════════════════════════════════════════╝
set -o pipefail
shopt -s extglob

BASE_URL="${BASE_URL}"    # may be empty for auto-detect
MAX_JOBS="${MAX_JOBS:-3}"
...
# ── 1. Safe env extraction ──
load_env_safe() {
  if [ -f .env.local ]; then
    # Only extract specific keys – no sourcing, no special char issues
    while IFS='=' read -r key value; do
      ...
      case "$key" in
        DATABASE_URL|TELEGRAM_BOT_TOKEN|TELEGRAM_CHAT_ID|ADMIN_EMAIL|ADMIN_PASSWORD)
          export "$key"="$value"
          ;;
      esac
    done < .env.local
...
# ── 2. Auto‑detect server if BASE_URL not set ──
if [ -z "$BASE_URL" ]; then
...
# ── 3. Auth token (if credentials exist) ──
AUTH_TOKEN=""
...
# ── 4. Fetch DB IDs (suppress errors) ──
fetch_id() {
...
wait_server() {
  for ((i=1;i<=30;i++)); do
    curl -s --max-time 2 "$BASE_URL/" >/dev/null 2>&1 && echo -e "${GREEN}✅ Server ready at $BASE_URL${NC}" && return 0
    sleep 1
  done
...
# ── 8. URL resolution with SKIP ──
resolve_url() {
  local u="$1"
  # list of placeholder:variable
  local pairs=(
    '\[uid\]' "$U"
    '\[vendorUid\]' "$V"
    '\[productId\]' "$PID"
    ...
    '\[jobId\]' "$CRAWLERJOBID"
    '\[convId\]' "$CONVID"
    '\[id\]' "${COUPONID:-${PID:-}}"   # fallback generic id
  )
  for ((i=0; i<${#pairs[@]}; i+=2)); do
    local placeholder="${pairs[$i]}"
    local value="${pairs[$i+1]}"
    if [[ "$u" == *"$placeholder"* ]]; then
      # If the placeholder is present but value is empty, SKIP
      if [ -z "$value" ]; then
        echo "SKIP"
        return
      fi
...
# ── 9. Test one URL (with retry, safe integer) ──
test_one() {
...
# ── Main ──
main() {
  echo -e "${PURPLE}⚡ INFINITY GOD MODE SCANNER v5.5 (Error‑Free)${NC}"
  wait_server || exit 1
  ...
  # Telegram message
  local telegram_msg="<b>⚡ Scan Complete (v5.5) ⚡</b>%0A"
