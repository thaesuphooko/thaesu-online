...
# Resolve URL – strict skip if any placeholder has empty value
resolve_url() {
  local u="$1"
  local pairs=(
...
# Test one URL with retry (skip 405 error, treat as info)
test_one() {
  ...
  if [ -z "$code" ] || [ "$code" = "000" ]; then fail="TIMEOUT"; icon="💀"; color="$RED"; code="000"
    ...
    elif [ "$code" = "405" ]; then fail="METHOD_NA"; icon="⚙️"; color="$BLUE"   # <-- 405 is now informational
...
main() {
  echo -e "${PURPLE}⚡ INFINITY GOD MODE SCANNER v5.7 (Smart Classification)${NC}"
  wait_server || exit 1
  discover
  ...
  echo -e "${PURPLE}══════════════════════════════════════════${NC}"
  echo -e "Total: ${total}  💀${crit}  ❌${errs}  ❓${miss}  ⚠️${warns}  ⏱️${slow}  🔒${auth}  ⚙️${methods}"
  local telegram_msg="<b>⚡ Scan Complete v5.7 ⚡</b>%0A<b>URL:</b> ${BASE_URL} | <b>Time:</b> ${elapsed}s%0A<b>Total:</b> ${total}  💀${crit}  ❌${errs}  ❓${miss}  ⚠️${warns}  ⏱️${slow}  🔒${auth}  ⚙️${methods}%0A"
