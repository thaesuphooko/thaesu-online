...
# ╔══════════════════════════════════════════════════════════╗
# ║  INFINITY GOD MODE SCANNER v5 (Telegram + Ultra Fast)  ║
# ╚══════════════════════════════════════════════════════════╝
...
# ─── Load .env.local for Telegram config ───
if [ -f .env.local ]; then
...
test_one() {
  ...
  local tmp=$(mktemp) code dur body attempt=0 max_attempts=2
...
# ─── Main ───
main() {
  echo -e "${PURPLE}⚡ INFINITY GOD MODE SCANNER v5 (Telegram Ready)${NC}"
  wait_server || exit 1
  ...
  telegram_msg+="<b>⏱️ Slow:</b> ${slow}%0A%0A"

  # Attach top 5 critical/error URLs if any
  if [ "$crit" -gt 0 ] || [ "$errs" -gt 0 ]; then
    telegram_msg+="<b>🚨 Top Issues:</b>%0A"
    grep -E "CRITICAL|ERROR" "$REPORT_TXT" | head -5 | while IFS= read -r line; do
  ...
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$TELEGRAM_CHAT_ID" ]; then
    ...
    echo -e "${YELLOW}⚠️  Telegram env not set. Skipping notification.${NC}"
  fi
