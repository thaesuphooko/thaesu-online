...
# Resolve URL – strict skip if any placeholder remains
resolve_url() {
  local u="$1"
  local pairs=(
  ...
  # Final safety: if URL still contains '[' after all replacements, skip
...
test_one() {
  ...
  if [ -z "$code" ] || [ "$code" = "000" ]; then fail="TIMEOUT"; icon="💀"; color="$RED"; code="000"
    ...
    elif [ "$code" = "400" ]; then fail="BAD_REQUEST"; icon="📝"; color="$YELLOW"   # 400 is client error, not server
...
main() {
  echo -e "${PURPLE}⚡ INFINITY GOD MODE SCANNER v5.8 (Perfect Skip + 400 Info)${NC}"
  wait_server || exit 1
  discover
  ...
  echo -e "${PURPLE}══════════════════════════════════════════${NC}"
  echo -e "Total: ${total}  💀${crit}  ❌${errs}  ❓${miss}  ⚠️${warns}  ⏱️${slow}  🔒${auth}  ⚙️${methods}  📝${badreq}"
  local telegram_msg="<b>⚡ Scan Complete v5.8 ⚡</b>%0A<b>URL:</b> ${BASE_URL} | <b>Time:</b> ${elapsed}s%0A<b>Total:</b> ${total}  💀${crit}  ❌${errs}  ❓${miss}  ⚠️${warns}  ⏱️${slow}  🔒${auth}  ⚙️${methods}  📝${badreq}%0A"
