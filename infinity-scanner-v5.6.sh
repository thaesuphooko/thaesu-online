...
resolve_url() {
  local u="$1"
  local pairs=('\[uid\]' "$U" '\[vendorUid\]' "$V" '\[productId\]' "$PID" '\[slug\]' "$SLUG" '\[...slug\]' "test-slug" '\[orderId\]' "$OID" '\[subscriptionId\]' "$SID" '\[postId\]' "$POSTID" '\[commentId\]' "$COMMENTID" '\[storyId\]' "$STORYID" '\[couponId\]' "$COUPONID" '\[botId\]' "$BOTID" '\[jobId\]' "$CRAWLERJOBID" '\[convId\]' "$CONVID" '\[id\]' "${COUPONID:-${PID:-}}")
...
# Fixed test_one with safe integer checks
test_one() {
  local method="$1" url="$2" data="$3" desc="$4"
  [ "$url" = "SKIP" ] && return
  local tmp=$(mktemp) code="" dur body auth_header="" use_auth=false attempt=0 max_attempts=2
  if [[ "$desc" == API* ]] && [ -n "$AUTH_TOKEN" ]; then use_auth=true; auth_header="-H \"Authorization: Bearer ${AUTH_TOKEN}\""; fi
  while [ $attempt -lt $max_attempts ]; do
    ...
    dur=$(( ($(date +%s%N) - start) / 1000000 )); body=$(cat "$tmp" 2>/dev/null)
    if [ "$code" = "000" ] || { [[ "$code" =~ ^[0-9]+$ ]] && [ "$code" -ge 500 ]; }; then attempt=$((attempt+1)); sleep 1; continue; fi
    break
  done
  rm -f "$tmp"
  ...
  if [ -z "$code" ] || [ "$code" = "000" ]; then fail="TIMEOUT"; icon="💀"; color="$RED"; code="000"
    ...
    if [ "$code" -ge 500 ]; then fail="CRITICAL"; icon="💀"; color="$RED"
      ...
      else fail="AUTH-REQUIRED"; icon="🔒"; color="$YELLOW"; fi
...
test_route_multi() {
  ...
  if [ "$type" = "PAGE" ]; then test_one GET "$route" "" "Page"
    ...
    test_one GET "$route" "" "API"; local pd='{"test":"true"}'
    [[ "$route" == *"/subscribe" ]] && pd='{"plan_id":"'${PLANID:-000}'"}'
    [[ "$route" == *"/login" ]] && pd='{"email":"x@x.com","password":"x"}'
    test_one POST "$route" "$pd" "API"; test_one PUT "$route" '{"test":"true"}' "API"; test_one DELETE "$route" "" "API"
  fi
}
...
main() {
  echo -e "${PURPLE}⚡ INFINITY GOD MODE SCANNER v5.6 (Integer Bug Fixed)${NC}"
  wait_server || exit 1
  discover
  ...
  local telegram_msg="<b>⚡ Scan Complete v5.6 ⚡</b>%0A<b>URL:</b> ${BASE_URL} | <b>Time:</b> ${elapsed}s%0A<b>Total:</b> ${total}  💀${crit}  ❌${errs}  ❓${miss}  ⚠️${warns}  ⏱️${slow}  🔒${auth}%0A"
  if [ "$crit" -gt 0 ] || [ "$errs" -gt 0 ]; then telegram_msg+="%0A<b>🚨 Top Issues:</b>%0A"; grep -E "CRITICAL|ERROR" "$REPORT_TXT" | head -5 | while IFS= read -r l; do telegram_msg+="<code>${l}</code>%0A"; done; fi
