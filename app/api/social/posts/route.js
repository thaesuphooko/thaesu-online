...
export async function GET(req) {
  try {
    const user = authenticate(req); // may be null if not logged in
    ...
    let result;
    if (user) {
      // Include a flag if current user liked the post
      result = await query(
        `SELECT p.*,
...
export async function POST(req) {
  ...
  try {
    ...
    post.user_name = userResult.rows[0]?.full_name || '';
    post.user_avatar = userResult.rows[0]?.avatar_url || '';
    post.user_uid = userResult.rows[0]?.uid || '';
